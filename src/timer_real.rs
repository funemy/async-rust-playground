...
impl Reactor {
    ...
    // Strengthen version:
    //  If a task is woken up, it must have satisfied its predicate.
    //  If a task is not woken up by the end of an iteration, it must be kept into the next iteration.
